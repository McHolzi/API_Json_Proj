...
def get_distance(origin, desitination):
    """
    Gets the distance of to provided points by using the google maps api
    args:
        origin[string]: place of origin for the drive, City or address.
        destination[string]: destination for the drive
    returns:
        response.__dict__ : dict of the distance matrix
    """
...
def get_sheet_values(sheetname):
    ...
    creds = ServiceAccountCredentials.from_json_keyfile_name(
        "google_api_proj\key_file.json", scope
    )
    ...
    data_relevant = data_all[
        data_all.columns[
            data_all.columns.isin(
                [
                    "Kursdatum",
                    "Ort",
                    "Datum",
    ...
    free_courses = data_relevant[
        data_relevant["Trainer"] == ""
    ]  # When there is nothing in the Trainer column, the course is still free
    my_courses = data_relevant[
        data_relevant["Trainer"] == trainer_name
    ]  # When my Name is in the Trainer column its already taken by me
...
def clean_sheet_values(sheet_values):
    ...
    i = sheet_values[
        (sheet_values.Ort == "")
    ].index  # index of the rows with no 'Ort' in them
    ...
    temp_new_dates = sheet_values[
        (sheet_values.Kursdatum.str.contains("/"))
    ]  # takes out only the rows with '/' in the date
    replace_values = temp_new_dates.Kursdatum.str.slice(
        start=3, stop=7
    )  # slices out the '/' and the Day of the date thats following
    ...
    for i in range(len(replace_values_list)):
        temp_new_dates = temp_new_dates.replace(
            temp_new_dates.iloc[i]["Kursdatum"],
            temp_new_dates.Kursdatum.str.replace(replace_values_list[i], "").iloc[i],
        )
    ...
    sheet_values = sheet_values.assign(
        Kursdatum=sheet_values["Kursdatum"].str.split("/")
    ).explode(
        "Kursdatum"
    )  # splits the dates with '/' in them
    sheet_values = sheet_values.reset_index(drop=True)
    index_newdate = sheet_values[
        (~sheet_values.Kursdatum.str.contains("2022"))
    ].index  # index of the dates that are incomplete
    sheet_values = sheet_values.drop(index_newdate, axis=0)  # drops the incomplete rows
    sheet_values = sheet_values.append(
        temp_new_dates
    )  # appends the corresponding complete rows
    sheet_values = sheet_values.reset_index(drop=True)

    #### adds new column with datetime timestamp, adds new column with Endtime (Beginn + Dauer)#####
    format_time_before = "%d.%m.%Y/%H:%M:%S%z"
    sheet_values["Starttime"] = (
        sheet_values["Kursdatum"] + "/" + sheet_values["Beginn"] + ":00" + "+02:00"
    )
    sheet_values["Starttime"] = sheet_values["Starttime"].apply(
        datetime.datetime.strptime, args=(format_time_before,)
    )
    ...
    sheet_values.loc[index_6h, "Endtime"] = pd.to_datetime(
        sheet_values["Starttime"].astype(str)
    ) + pd.DateOffset(hours=6)
    index_8h = sheet_values[(sheet_values.Dauer == 8)].index
    sheet_values.loc[index_8h, "Endtime"] = pd.to_datetime(
        sheet_values["Starttime"].astype(str)
    ) + pd.DateOffset(hours=8)
    sheet_values.loc[sheet_values.Endtime.isnull(), "Endtime"] = sheet_values.loc[
        sheet_values.Endtime.isnull(), "Starttime"
    ] + pd.DateOffset(hours=5)
...
def get_creds_calendar():
    ...
    # time.
    if os.path.exists(
        r"C:\Users\Marco Holzer\API_Json_Proj\google_api_proj\token.json"
    ):
        creds = Credentials.from_authorized_user_file(
            r"C:\Users\Marco Holzer\API_Json_Proj\google_api_proj\token.json", SCOPES
        )
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            flow = InstalledAppFlow.from_client_secrets_file(
                r"C:\Users\Marco Holzer\API_Json_Proj\google_api_proj\credentials.json",
                SCOPES,
            )
            creds = flow.run_local_server(port=0)
        # Save the credentials for the next run
        with open(
            r"C:\Users\Marco Holzer\API_Json_Proj\google_api_proj\token.json", "w"
        ) as token:
...
def change_colour_events(
    event_count, colorid, main_key="Schulung Burgenland", alternative_key="Kurs!"
):
    """
    checks in calendar if the events which are courses are the right color, if not, changes it.
    args:
        event_count[int]: number of coming events to be checked
        colorid[int]: id of the color used, this is google calendar specific:
            1 Lavender
            2 Sage
            3 Grape
            4 Flamingo
            ...
            11 Tomato
        kwargs:
            main_key[string]: the key, for the courses, this is the organizer in the calendar event.
            alternative_key[string]: if the organizer is not specified, this will be used to identify the courses by checking the name (summary) of the events, default= Kurs!,
                                    if you do not want to use this key, set it to a character that you know is not used in the summary of your events
    """
    creds = get_creds_calendar()
    ...
    for event in events:
        ...
        if (
            "colorId" in event
            and event["colorId"] != colorid
            and (
                event["organizer"].get("displayName") == main_key
                or alternative_key in event["summary"]
            )
        ):
            try:
                service = build("calendar", "v3", credentials=creds)
    ...
    for event in events:
        ...
        if organizer == organizer_name and "Dauer" not in event_last["summary"]:
            ...
            starttime = datetime.datetime.strptime(
                event["start"].get("dateTime"), format_time
            )
            starttime_drive = starttime - buffer_time
            place = event["location"]
            distance = get_values_maps(address, place)
            dist_json = json.loads(distance)
            driving_time = datetime.timedelta(
                minutes=round(
                    dist_json["rows"][0]["elements"][0]["duration"]["value"] / 60
                )
            )
            ...
            print(
                ...
                + " Dauer:"
                + str(
                    round(dist_json["rows"][0]["elements"][0]["duration"]["value"] / 60)
                )
            )
...
def compare_calendar_sheets(free_courses_sheets, events_calendar):
    ...
    event_df_final["Dauer"] = pd.to_datetime(
        event_df_final["Endtime"]
    ) - pd.to_datetime(event_df_final["Starttime"])
